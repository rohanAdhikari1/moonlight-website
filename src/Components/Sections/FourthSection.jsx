        ...
        <div className='min-h-screen dark:bg-black bg-gray-50 dark:border-b-2 dark:border-white pb-16'>
            ...
            <div className='md:px-16 px-4 py-3'>
                <div className='flex flex-wrap justify-around gap-5 lg:gap-0'>
                    {tabs.map((value, key) =>
                    (<div key={key} onClick={() => setactivetab(key)} className="lg:w-1/5 cursor-pointer md:w-1/3 w-full min-h-40 lg:gap-0 lg:px-3">
                        <div className="relative w-full h-full flex">
                            <div className={`${activetab == key ? 'bg-red-500 dark:bg-red-500' : 'dark:bg-zinc-800 ' + (value.bg || 'bg-pink-200')} h-full max-h-[160px] w-full rounded-xl`}>
                                <div className="flex items-center justify-center flex-col h-full">
                                    {value.icon && <img className={`object-cover h-1/3 ${activetab == key ? 'invert' : 'invert-0'} dark:invert`} src={value.icon || "default.webp"} alt={value.name} />}
                    ...
                    {tabs.map((val, key) => activetab == key && (
                        <div key={key} className='grid grid-cols-1 md:grid-cols-2 gap-8'>
                            <motion.div variants={SlideInRightAnimationVariants}
                                ...
                                <img src={val.image || "default.webp"} alt={val.name || "Image"} className="max-w-full rounded-2xl h-auto" />
                            </motion.div>
                            ...
                            <motion.div variants={SlideInLeftAnimationVariants}
                                ...
                                <div className="p-4">
                                    ...
                                    <p className='font-quicksand text-lg leading-10 dark:text-white'>
                                        {(tabs[activetab].description).length > 400 ? tabs[activetab].description.substring(0, 400) + "..." : tabs[activetab].description}
                                    </p>
                                    <div>
                                        <Link to={`service/${activetab}`}><button
                                        ...
                                        rounded-xl my-8 py-3 px-4 dark:text-white dark:border-white dark:border-2 shadow-2xl hover:bg-red-500 text-white transition duration-500 transform translate-x-1 translate-y-1 hover:border-red-500 ease-out shadow-primary hover:shadow-red-500'>
                                            Read More <MdArrowRightAlt className='text-2xl mt-[1px] ms-[1px]' /></button></Link>
                                    </div>
                                </div>
                            </motion.div>
                        </div>
